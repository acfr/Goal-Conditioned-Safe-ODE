...
# Calculate the quadratic with global minimum encoded
# 0.5*|g(x)-g(x_opt)|^2
# encode the minimum point in structure
class QuadraticPoentialOptimal(nn.Module):
    BiLipBlock: nn.Module
    x_optimal: jnp.array
    add_constant: float = False
    ...
    def __call__(self, x: jnp.array, ) -> jnp.array:
        gv_x = self.BiLipBlock(x)
        gv_x_optimal = self.BiLipBlock(self.x_optimal)
        y = QuadPotential(add_constant = self.add_constant)(gv_x - gv_x_optimal)
